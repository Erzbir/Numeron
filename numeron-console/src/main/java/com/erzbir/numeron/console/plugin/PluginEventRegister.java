        ...
        pluginContext.getClasses().forEach(t -> {
            if (t.isAnnotationPresent(Listener.class)) {
                ...
                for (Method declaredMethod : declaredMethods) {
                    if (!declaredMethod.isAnnotationPresent(Message.class) && !declaredMethod.isAnnotationPresent(Event.class)) {
                        continue;
                    }
                    Message message = declaredMethod.getAnnotation(Message.class);
                    Event eventAnnotation = declaredMethod.getAnnotation(Event.class);
                    if (message != null) {
